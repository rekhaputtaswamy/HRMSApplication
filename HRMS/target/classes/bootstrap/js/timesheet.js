...
$(document).ready(function(){
	$(function(){
	    var projectslist = $('#proj0').children('option').map(function(k,e){
	       return e.innerText;
	    }).get();
	    for(var k = 0; k < projectslist.length; k++) {
	    	projects += "<option id='p" + k+1 + "'>" + projectslist[k] + "</option>";
	    }
	    
	    var taskslist = $('#task0').children('option').map(function(k,e){
		       return e.innerText;
		    }).get();
	    for(var k = 0; k < taskslist.length; k++) {
	    	tasks += "<option id='t" + k+1 + "'>" + taskslist[k] + "</option>";
	    }
	    
	    var desclist = $('#desc0').children('option').map(function(k,e){
		       return e.innerText;
		    }).get();
	    
	    for(var k = 0; k < desclist.length; k++) {
	    	desc += "<option id='d" + k+1 + "'>" + desclist[k] + "</option>";
	    }
		...
		$(this).find("td").each(function(){
			...
			if($(this).find('select').val() == undefined) {
				...
				if($(this).find('select').val() == 'Select') {
					BootstrapDialog.alert("Enter Project Data");
					status = false;
					return false;
				}
...
function updateTotal(id, value) {
	...
	if (id.startsWith("mon_st")) {
		mon_values_st[parseInt(id.slice(-1))] = parseFloat(value);
		var mon_st_total = getTotal(mon_values_st);
		$('#mon_st_total').val(parseFloat(mon_st_total));
		
 	} else if (id.startsWith("mon_ot")) {
		mon_values_ot[parseInt(id.slice(-1))] = parseFloat(value);
		var mon_ot_total = getTotal(mon_values_ot);
		$('#mon_ot_total').val(parseFloat(mon_ot_total));
		
 	} else if (id.startsWith("mon_nb")) {
		mon_values_nb[parseInt(id.slice(-1))] = parseFloat(value);
		var mon_nb_total = getTotal(mon_values_nb);
		$('#mon_nb_total').val(parseFloat(mon_nb_total));
		
 	} else if (id.startsWith("tue_st")) {
 		tue_values_st[parseInt(id.slice(-1))] = parseFloat(value);
		var tue_st_total = getTotal(tue_values_st);
		$('#tue_st_total').val(parseFloat(tue_st_total));
		
 	} else if (id.startsWith("tue_ot")) {
 		tue_values_ot[parseInt(id.slice(-1))] = parseFloat(value);
		var tue_ot_total = getTotal(tue_values_ot);
		$('#tue_ot_total').val(parseFloat(tue_ot_total));
		
 	} else if (id.startsWith("tue_nb")) {
 		tue_values_nb[parseInt(id.slice(-1))] = parseFloat(value);
		var tue_nb_total = getTotal(tue_values_nb);
		$('#tue_nb_total').val(parseFloat(tue_nb_total));
		
 	} else if (id.startsWith("wed_st")) {
 		wed_values_st[parseInt(id.slice(-1))] = parseFloat(value);
		var wed_st_total = getTotal(wed_values_st);
		$('#wed_st_total').val(parseFloat(wed_st_total));
		
 	} else if (id.startsWith("wed_ot")) {
 		wed_values_ot[parseInt(id.slice(-1))] = parseFloat(value);
		var wed_ot_total = getTotal(wed_values_ot);
		$('#wed_ot_total').val(parseFloat(wed_ot_total));
		
 	} else if (id.startsWith("wed_nb")) {
 		wed_values_nb[parseInt(id.slice(-1))] = parseFloat(value);
		var wed_nb_total = getTotal(wed_values_nb);
		$('#wed_nb_total').val(parseFloat(wed_nb_total));
		
 	} else if (id.startsWith("thu_st")) {
 		thu_values_st[parseInt(id.slice(-1))] = parseFloat(value);
		var thu_st_total = getTotal(thu_values_st);
		$('#thu_st_total').val(parseFloat(thu_st_total));
		
 	} else if (id.startsWith("thu_ot")) {
 		thu_values_ot[parseInt(id.slice(-1))] = parseFloat(value);
		var thu_ot_total = getTotal(thu_values_ot);
		$('#thu_ot_total').val(parseFloat(thu_ot_total));
		
 	} else if (id.startsWith("thu_nb")) {
 		thu_values_nb[parseInt(id.slice(-1))] = parseFloat(value);
		var thu_nb_total = getTotal(thu_values_nb);
		$('#thu_nb_total').val(parseFloat(thu_nb_total));
		
 	} else if (id.startsWith("fri_st")) {
 		fri_values_st[parseInt(id.slice(-1))] = parseFloat(value);
		var fri_st_total = getTotal(fri_values_st);
		$('#fri_st_total').val(parseFloat(fri_st_total));
		
 	} else if (id.startsWith("fri_ot")) {
 		fri_values_ot[parseInt(id.slice(-1))] = parseFloat(value);
		var fri_ot_total = getTotal(fri_values_ot);
		$('#fri_ot_total').val(parseFloat(fri_ot_total));
		
 	} else if (id.startsWith("fri_nb")) {
 		fri_values_nb[parseInt(id.slice(-1))] = parseFloat(value);
		var fri_nb_total = getTotal(fri_values_nb);
		$('#fri_nb_total').val(parseFloat(fri_nb_total));
		
 	} else if (id.startsWith("sat_st")) {
 		sat_values_st[parseInt(id.slice(-1))] = parseFloat(value);
		var sat_st_total = getTotal(sat_values_st);
		$('#sat_st_total').val(parseFloat(sat_st_total));
		
 	} else if (id.startsWith("sat_ot")) {
 		sat_values_ot[parseInt(id.slice(-1))] = parseFloat(value);
		var sat_ot_total = getTotal(sat_values_ot);
		$('#sat_ot_total').val(parseFloat(sat_ot_total));
		
 	} else if (id.startsWith("sat_nb")) {
 		sat_values_nb[parseInt(id.slice(-1))] = parseFloat(value);
		var sat_nb_total = getTotal(sat_values_nb);
		$('#sat_nb_total').val(parseFloat(sat_nb_total));
		
 	} else if (id.startsWith("sun_st")) {
 		sun_values_st[parseInt(id.slice(-1))] = parseFloat(value);
		var sun_st_total = getTotal(sun_values_st);
		$('#sun_st_total').val(parseFloat(sun_st_total));
		
 	} else if (id.startsWith("sun_ot")) {
 		sun_values_ot[parseInt(id.slice(-1))] = parseFloat(value);
		var sun_ot_total = getTotal(sun_values_ot);
		$('#sun_ot_total').val(parseFloat(sun_ot_total));
		
 	} else if (id.startsWith("sun_nb")) {
 		sun_values_nb[parseInt(id.slice(-1))] = parseFloat(value);
		var sun_nb_total = getTotal(sun_values_nb);
		$('#sun_nb_total').val(parseFloat(sun_nb_total));
...
function deleteRow() {
	...
	$('#mon_st_total').val(parseFloat(getTotal(mon_values_st)));
	$('#mon_ot_total').val(parseFloat(getTotal(mon_values_ot)));
	$('#mon_nb_total').val(parseFloat(getTotal(mon_values_nb)));
	$('#tue_st_total').val(parseFloat(getTotal(tue_values_st)));
	$('#tue_ot_total').val(parseFloat(getTotal(tue_values_ot)));
	$('#tue_nb_total').val(parseFloat(getTotal(tue_values_nb)));
	$('#wed_st_total').val(parseFloat(getTotal(wed_values_st)));
	$('#wed_ot_total').val(parseFloat(getTotal(wed_values_ot)));
	$('#wed_nb_total').val(parseFloat(getTotal(wed_values_nb)));
	$('#thu_st_total').val(parseFloat(getTotal(thu_values_st)));
	$('#thu_ot_total').val(parseFloat(getTotal(thu_values_ot)));
	$('#thu_nb_total').val(parseFloat(getTotal(thu_values_nb)));
	$('#fri_st_total').val(parseFloat(getTotal(fri_values_st)));
	$('#fri_ot_total').val(parseFloat(getTotal(fri_values_ot)));
	$('#fri_nb_total').val(parseFloat(getTotal(fri_values_nb)));
	$('#sat_st_total').val(parseFloat(getTotal(sat_values_st)));
	$('#sat_ot_total').val(parseFloat(getTotal(sat_values_ot)));
	$('#sat_nb_total').val(parseFloat(getTotal(sat_values_nb)));
	$('#sun_st_total').val(parseFloat(getTotal(sun_values_st)));
	$('#sun_ot_total').val(parseFloat(getTotal(sun_values_ot)));
	$('#sun_nb_total').val(parseFloat(getTotal(sun_values_nb)));
...
function setInTimeTotal() {
	var sams_total = parseFloat($('#day1_sams').val()) + parseFloat($('#day2_sams').val()) + parseFloat($('#day3_sams').val()) + 
					 parseFloat($('#day4_sams').val()) + parseFloat($('#day5_sams').val()) + parseFloat($('#day6_sams').val()) +
					 parseFloat($('#day7_sams').val());
	
	$('#all_week_in').val(parseFloat(sams_total));
}
...
function timeValidation() {
	...
	var mon_sams = 0;
	if($('#day1_sams').val() != '') {
		mon_sams = parseFloat($('#day1_sams').val());
	}
	if(mon_sams != 0 && mon > mon_sams) {
		showAlert("Monday");
		return false;
	}
	...
	var tue_sams = 0;
	if($('#day2_sams').val() != '') {
		tue_sams = parseFloat($('#day2_sams').val());
	}
	if(tue_sams != 0 && tue > tue_sams) {
		showAlert("Tuesday");
		return false;
	}
	...
	var wed_sams = 0;
	if($('#day3_sams').val() != '') {
		wed_sams = parseFloat($('#day3_sams').val());
	}
	if(wed_sams != 0 && wed > wed_sams) {
		showAlert("Wednesday");
		return false;
	}
	...
	var thu_sams = 0;
	if($('#day4_sams').val() != '') {
		thu_sams = parseFloat($('#day4_sams').val());
	}
	if(thu_sams != 0 && thu > thu_sams) {
		showAlert("Thursday");
		return false;
	}
	...
	var fri_sams = 0;
	if($('#day5_sams').val() != '') {
		fri_sams = parseFloat($('#day5_sams').val());
	}
	if(fri_sams != 0 && fri > fri_sams) {
		showAlert("Friday");
		return false;
	}
	...
	var sat_sams = 0;
	if($('#day6_sams').val() != '') {
		sat_sams = parseFloat($('#day6_sams').val());
	}
	if(sat_sams != 0 && sat > sat_sams) {
		showAlert("Saturday");
		return false;
	}
	...
	var sun_sams = 0;
	if($('#day7_sams').val() != '') {
		sun_sams = parseFloat($('#day7_sams').val());
	}
	if(sun_sams != 0 && sun > sun_sams) {
		showAlert("Sunday");
		return false;
	}
...
function getDayTotal(day) {
	...
	if($(stId).val() != '') {
		dayTotal += parseFloat($(stId).val());
	}
	
	if($(otId).val() != '') {
		dayTotal += parseFloat($(otId).val());
	}
	
	if($(nbId).val() != '') {
		dayTotal += parseFloat($(nbId).val());
	}
